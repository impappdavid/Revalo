...
export default function ClipScroller() {
    ...
    useEffect(() => {
        ...
        // Update URL query param for clip
        const clip = clips[current];
        if (clip) {
            const url = new URL(window.location.href);
            url.searchParams.set("clip", clip.id.toString());
            window.history.replaceState({}, "", url.toString());
        }
    }, [current, isMuted, volume]);

    // Update video volume/mute when changed or current clip changes
    useEffect(() => {
        const video = videoRefs.current[current];
        if (!video) return;
        video.muted = isMuted;
    ...
    // Toggle mute/unmute button click
    function toggleMute() {
        if (isMuted) {
            setIsMuted(false);
                ...
                {showVolumeSlider && (
                    <div className="w-24">
                        <Slider
                            max={1}
                            step={0.01}
                            value={isMuted ? [0] : [volume]}        // Pass value as array
                            onValueChange={(val: number[]) => {    // Receive array and extract first element
                                const newVol = val[0];
                                setVolume(newVol);
                                setIsMuted(newVol === 0);
                            }}
